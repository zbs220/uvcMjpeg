        ...
        int buf_size)
{
    ...
    else
    {
        ...
        if (pcur < plimit)
        {
            ...
            //printf("0: jpeg_buf size = %d\n", buf_size + sizeof(dht_data) + 10);
            if (jpeg_buf != NULL)
            {
#ifdef DECODER_DEBUG
                memset(jpeg_buf, 0, buf_size + sizeof(dht_data) + 10);
#endif
                ...
                //printf("2: pos = %d\n", pos);
                //printf("2: buf_size = %d\n", buf_size);
                //printf("2: size_start = %d\n", size_start);
                //printf("2: buf_size - size_start = %d\n", buf_size - size_start);
                //printf("2: pcur = %d\n", countTmp);

                //memcpy(jpeg_buf + pos, pcur, buf_size - size_start);  //原始代码
                //memcpy(jpeg_buf + pos, pcur, 153000);
                memcpy(jpeg_buf + pos, pcur, 614225);      //此处如果用源代码，会有段错误，但是没有定位出具体原因，先使用具体值
...
Decoder *decoder_mjpeg_create(void)
{
